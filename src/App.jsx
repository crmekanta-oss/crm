...
/*
  ...
  EKANTA CRM — v5
  New in v5:
  ⑦ Follow-up Activity Log — all roles can log outcomes
     per follow-up date + optionally set next follow-up
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
*/
...
// ─── DESIGN TOKENS ────────────────────────────────────────────────────────────
const T = {
  bg:"#F7F8FA", surface:"#FFFFFF", surfaceHover:"#FAFAFA", sidebar:"#FFFFFF",
  brand:"#5B3BE8", brandHover:"#4A2CC5", brandSubtle:"#F0EEFF",
  ink:"#0F0F10", inkSub:"#6E6E80", inkMuted:"#A1A1AA", inkInvert:"#FFFFFF",
  line:"rgba(0,0,0,0.07)", lineMid:"rgba(0,0,0,0.11)", lineStrong:"rgba(0,0,0,0.16)",
...
function xls(data,name){
  ...
  const rows=data.map((f,i)=>{
    const prod=(f.products||[]).map(p=>`${p.desc}(${p.category},x${p.qty},Rs${p.price})`).join("|");
    return `<Row>${[[i+1,"Number"],[f.name||""],[f.leadSource||""],[f.phone||""],[f.email||""],[f.enquiryType||""],[f.funnelType||""],[f.nextFollowUp||""],[f.status],[f.cityRegion||""],[f.deliveryDetails||""],[f.paymentTerms||""],[prod],[f.orderNumber||""],[f.quoteQty||"",f.quoteQty?"Number":"String"],[f.quoteAmount||"",f.quoteAmount?"Number":"String"],[f.remarks||""],[f.createdAt],[f.createdBy]].map(([v,t="String"])=>`<Cell><Data ss:Type="${t}">${e(v)}</Data></Cell>`).join("")}</Row>`;
  }).join("");
...
function StatusPill({status,sm}){
  const map={Won:T.won,Pending:T.pending,Lost:T.lost,Drop:T.drop,"New Lead":T.new,"Qualified":T.pending,"Proposal Sent":T.high,"High Value":T.high,"Premium":T.premium,"Bulk":T.bulk,"Normal":T.drop,"Strategic":T.new};
...
function SourcePill({source}){
  ...
  return(
    <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"1px 7px",borderRadius:20,fontSize:10,fontWeight:500,background:"rgba(0,0,0,0.04)",color:T.inkSub,fontFamily:F,whiteSpace:"nowrap",border:`1px solid ${T.line}`}}>
      <span style={{fontSize:10,lineHeight:1}}>{icons[source]||"•"}</span>{source}
    </span>
  );
}
...
function Btn({primary,ghost,danger,sm,icon,label,onClick,disabled,full,iconRight}){
  ...
  const styles={...(primary?{background:hov?T.brandHover:T.brand,color:"#fff",boxShadow:hov?"0 2px 8px rgba(91,59,232,.3)":T.shadowSm}:{}),...(ghost?{background:hov?T.surfaceHover:T.surface,color:T.ink,border:`1px solid ${T.line}`,boxShadow:hov?T.shadowSm:"none"}:{}),...(danger?{background:hov?"#FEF2F2":T.surface,color:"#B91C1C",border:`1px solid ${hov?"#FECACA":T.line}`}:{}),...(!primary&&!ghost&&!danger?{background:hov?T.surfaceHover:"transparent",color:T.inkSub,border:"1px solid transparent"}:{})};
  return(
    <button onClick={disabled?undefined:onClick} disabled={disabled} style={{...base,...styles}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} onMouseDown={e=>!disabled&&(e.currentTarget.style.transform="scale(0.97)")} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"}>
...
// ─── INPUT PRIMITIVES ─────────────────────────────────────────────────────────
const inputSx=(err)=>({width:"100%",padding:"8px 11px",border:`1px solid ${err?"#FECACA":T.lineMid}`,borderRadius:T.r.md,fontSize:13,fontFamily:F,color:T.ink,background:T.surface,outline:"none",boxSizing:"border-box",transition:"border-color .15s,box-shadow .15s"});
...
function FInput({label,value,onChange,placeholder,type="text",error,required,full=true,style:sx}){
  return(
    <div style={{display:"flex",flexDirection:"column",gap:5,...(full?{}:{flex:1})}}>
      ...
      <input type={type} value={value} onChange={onChange} placeholder={placeholder} style={{...inputSx(error),...sx}} onFocus={onfocus} onBlur={onblur}/>
...
function FSelect({label,value,onChange,options,placeholder,full=true,required,error}){
  return(
    <div style={{display:"flex",flexDirection:"column",gap:5,...(full?{}:{flex:1})}}>
      {label&&<label style={{fontSize:12,fontWeight:500,color:T.inkSub,fontFamily:F}}>{label}{required&&<span style={{color:"#DC2626",marginLeft:2}}>*</span>}</label>}
      <select value={value} onChange={onChange} style={{...inputSx(error),cursor:"pointer",appearance:"none",background:`${T.surface} ${selectBg}`}} onFocus={onfocus} onBlur={e=>{e.target.style.borderColor=error?"#FECACA":T.lineMid;e.target.style.boxShadow="none";}}>
...
function Login({users,onLogin}){
  ...
  const go=()=>{
    const tu=u.trim().toLowerCase(), tp=p.trim();
    if(!tu||!tp){se("Please fill in all fields.");return;}
    se(""); sl(true);
    setTimeout(()=>{
      const found=users.find(x=>x.username.toLowerCase()===tu&&x.password===tp);
      if(found) onLogin(found); else{se("Incorrect username or password.");sl(false);}
    },600);
  };
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:T.bg,fontFamily:F,padding:24}}>
      <div style={{width:"100%",maxWidth:380,animation:"fadeUp .3s ease"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:32}}>
          <div style={{width:34,height:34,background:T.brand,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={P.layers} sz={16} color="#fff" sw={2}/></div>
        ...
        <div style={{background:T.surface,border:`1px solid ${T.line}`,borderRadius:T.r["2xl"],padding:"32px 28px",boxShadow:T.shadowLg}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              ...
              <input type="password" value={p} onChange={e=>sp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Enter password" style={{...inputSx()}} onFocus={onfocus} onBlur={onblur}/>
            </div>
            <button onClick={go} disabled={load} style={{width:"100%",padding:"10px",background:load?T.brandHover:T.brand,color:"#fff",border:"none",borderRadius:T.r.md,fontSize:14,fontWeight:600,fontFamily:F,cursor:load?"not-allowed":"pointer",transition:"background .15s",marginTop:4}} onMouseDown={e=>!load&&(e.currentTarget.style.transform="scale(0.99)")} onMouseUp={e=>e.currentTarget.style.transform="scale(1)"}>
...
function Sidebar({active,set,user,onLogout,open,onClose}){
  const nav=[{id:"dashboard",label:"Dashboard",icon:P.dash},{id:"funnels",label:"Funnels",icon:P.list},...(FULL.includes(user.role)?[{id:"analytics",label:"Analytics",icon:P.chart},{id:"team",label:"Team",icon:P.users}]:[])];
  return(
    <>
      ...
      <div className={`ek-sidebar${open?" open":""}`} style={{width:200,minHeight:"100vh",background:T.sidebar,borderRight:`1px solid ${T.line}`,display:"flex",flexDirection:"column",flexShrink:0,position:"relative",zIndex:200}}>
        <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${T.line}`}}>
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            <div style={{width:28,height:28,background:T.brand,borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={P.layers} sz={13} color="#fff" sw={2.2}/></div>
            <div><div style={{fontSize:13,fontWeight:700,color:T.ink,letterSpacing:"-0.2px",lineHeight:1.2}}>Ekanta</div><div style={{fontSize:10,color:T.inkMuted,lineHeight:1}}>Design Studio</div></div>
          </div>
        </div>
        <nav style={{flex:1,padding:"8px 8px 0"}}>
          ...
          {nav.map(item=>{
            ...
            return(
              <button key={item.id} onClick={()=>{set(item.id);onClose&&onClose();}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"7px 8px",borderRadius:T.r.md,border:"none",background:a?"rgba(91,59,232,.06)":"transparent",color:a?T.brand:T.inkSub,fontFamily:F,fontSize:13,fontWeight:a?600:400,cursor:"pointer",marginBottom:1,transition:"all .12s",textAlign:"left",position:"relative"}} onMouseEnter={e=>{if(!a){e.currentTarget.style.background=T.bg;e.currentTarget.style.color=T.ink;}}} onMouseLeave={e=>{if(!a){e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.inkSub;}}}>
                ...
                <Ic d={item.icon} sz={14} color={a?T.brand:T.inkSub}/>{item.label}
              </button>
            );
          })}
        </nav>
        <div style={{padding:"10px 8px 14px",borderTop:`1px solid ${T.line}`}}>
          <div style={{display:"flex",alignItems:"center",gap:9,padding:"7px 8px",borderRadius:T.r.md}}>
            ...
            <button onClick={onLogout} title="Sign out" style={{background:"none",border:"none",cursor:"pointer",color:T.inkMuted,display:"flex",padding:3,borderRadius:5,transition:"color .15s"}} onMouseEnter={e=>e.currentTarget.style.color=T.ink} onMouseLeave={e=>e.currentTarget.style.color=T.inkMuted}>
    ...
    <div style={{background:T.surface,borderBottom:`1px solid ${T.line}`,padding:"0 16px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:56}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flex:1}}>
          <button onClick={onMenuToggle} style={{background:"none",border:"none",cursor:"pointer",color:T.inkSub,padding:4,borderRadius:6}} onMouseEnter={e=>e.currentTarget.style.color=T.ink} onMouseLeave={e=>e.currentTarget.style.color=T.inkSub}>
          ...
          <div className="ek-topbar-search" style={{display:"flex",alignItems:"center",gap:8,background:T.bg,border:`1px solid ${T.line}`,borderRadius:T.r.md,padding:"6px 11px",minWidth:220,maxWidth:320,flex:1}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search funnels…" style={{border:"none",background:"transparent",outline:"none",fontSize:13,color:T.ink,fontFamily:F,width:"100%"}}/>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center",marginLeft:12,flexWrap:"wrap"}}>
          {FULL.includes(user.role)&&<><Btn ghost sm icon={P.dl} label={`Filtered (${fLen})`} onClick={onExportFiltered}/><Btn ghost sm icon={P.dl} label={`All (${aLen})`} onClick={onExportAll}/></>}
...
function Stats({funnels}){
  ...
  const s={total:funnels.length,won:won.length,pending:funnels.filter(f=>f.status==="Pending").length,lost:funnels.filter(f=>f.status==="Lost").length,revenue:won.reduce((a,f)=>a+(Number(f.quoteAmount)||0),0)};
  const wr=s.total?Math.round(s.won/s.total*100):0;
  const cards=[{label:"Total leads",value:s.total,caption:"All leads",accent:T.inkMuted},{label:"Won",value:s.won,caption:`${wr}% win rate`,accent:T.won.dot},{label:"Pending",value:s.pending,caption:"Need follow-up",accent:T.pending.dot},{label:"Lost",value:s.lost,caption:"Closed lost",accent:T.lost.dot},{label:"Won Revenue",value:big(s.revenue),caption:"Based on quotes",accent:T.won.dot}];
  return(
    <div className="ek-stats-grid" style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12,padding:"20px 24px 0"}}>
      {cards.map((c,i)=>(
        <div key={i} style={{background:T.surface,border:`1px solid ${T.line}`,borderRadius:T.r.lg,padding:"16px 18px",boxShadow:T.shadowSm,animation:`fadeUp .25s ease ${i*.04}s both`}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5}}><Dot color={c.accent} size={5}/><span style={{fontSize:11,color:T.inkMuted,fontFamily:F}}>{c.caption}</span></div>
        </div>
      ))}
    </div>
  );
...
function FilterBar({fil,setF,reset}){
  const sel=(val,key,opts)=>(
    <select value={val} onChange={e=>setF(key,e.target.value)} style={{padding:"5px 24px 5px 9px",border:`1px solid ${T.line}`,borderRadius:T.r.md,fontSize:12,fontFamily:F,color:val?T.ink:T.inkSub,background:val?`${T.brandSubtle} ${selectBg}`:`${T.surface} ${selectBg}`,cursor:"pointer",outline:"none",appearance:"none",fontWeight:val?500:400}}>
  ...
  const chk=(key,label)=>(
    <label style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:fil[key]?T.brand:T.inkSub,cursor:"pointer",fontFamily:F,fontWeight:fil[key]?500:400,userSelect:"none",transition:"color .15s"}}>
      <input type="checkbox" checked={fil[key]} onChange={e=>setF(key,e.target.checked)} style={{accentColor:T.brand,width:13,height:13}}/>{label}
    </label>
  );
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 24px",borderBottom:`1px solid ${T.line}`,background:T.surface,flexWrap:"wrap"}}>
      ...
      <div style={{width:1,height:14,background:T.line}}/>
      {sel(fil.status,"status",STATUS)}{sel(fil.funnelType,"funnelType",FTYPES)}{sel(fil.enquiryType,"enquiryType",ENQS)}{sel(fil.leadSource,"leadSource",LEAD_SOURCES)}
      <div style={{width:1,height:14,background:T.line}}/>
      <div style={{display:"flex",alignItems:"center",gap:7,background:T.bg,border:`1px solid ${T.line}`,borderRadius:T.r.md,padding:"4px 10px",minWidth:180}}>
        ...
        <input value={fil.descFilter} onChange={e=>setF("descFilter",e.target.value)} placeholder="Search description…" style={{border:"none",background:"transparent",outline:"none",fontSize:12,color:T.ink,fontFamily:F,width:"100%"}}/>
...
function Table({rows,user,onView,onEdit,onDelete,loading}){
  ...
  if(!rows.length) return(
    <div style={{textAlign:"center",padding:"72px 24px",fontFamily:F}}>
      <div style={{width:48,height:48,background:T.brandSubtle,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}><Ic d={P.list} sz={22} color={T.brand}/></div>
  ...
  return(
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:F,tableLayout:"fixed"}}>
        <colgroup><col style={{width:"3%"}}/><col style={{width:"20%"}}/><col style={{width:"15%"}}/><col style={{width:"12%"}}/><col style={{width:"12%"}}/><col style={{width:"12%"}}/><col style={{width:"12%"}}/><col style={{width:"14%"}}/></colgroup>
        <thead><tr><TH ch="#"/><TH ch="Name"/><TH ch="Category"/><TH ch="Type"/><TH ch="Follow-up"/><TH ch="Status"/><TH ch="Order Number"/><TH ch="Quote"/><TH ch=""/></tr></thead>
        <tbody>
          {rows.map((f,i)=>{
            ...
            return(
              <tr key={f.id} style={{borderBottom:`1px solid ${T.line}`,transition:"background .1s",cursor:"default"}} onMouseEnter={e=>e.currentTarget.style.background=T.bg} onMouseLeave={e=>e.currentTarget.style.background=T.surface}>
                ...
                <td style={{padding:"0 14px",verticalAlign:"middle",overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3}}><span style={{fontSize:10,color:T.inkMuted}}>{f.createdBy}</span>{f.leadSource&&<SourcePill source={f.leadSource}/>}</div>
                </td>
                ...
                <td style={{padding:"0 14px",verticalAlign:"middle"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5}}>{over&&<Dot color={T.lost.dot} size={5}/>}{tod&&<Dot color={T.pending.dot} size={5}/>}<span style={{fontSize:12,color:over?"#B91C1C":tod?T.pending.text:T.inkSub,fontWeight:over||tod?600:400}}>{f.nextFollowUp||"—"}</span></div>
                ...
                <td style={{padding:"0 10px",verticalAlign:"middle"}}>
                  <div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
                    <button onClick={()=>onView(f)} style={{background:"transparent",border:`1px solid ${T.line}`,borderRadius:T.r.sm,padding:"4px 10px",fontSize:11,fontWeight:500,color:T.inkSub,cursor:"pointer",fontFamily:F,transition:"all .12s"}} onMouseEnter={e=>{e.currentTarget.style.background=T.brandSubtle;e.currentTarget.style.color=T.brand;e.currentTarget.style.borderColor=T.brand;}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.inkSub;e.currentTarget.style.borderColor=T.line;}}>View</button>
                    {can(user,"edit")&&<button onClick={()=>onEdit(f)} style={{background:"transparent",border:`1px solid ${T.line}`,borderRadius:T.r.sm,padding:"4px 6px",cursor:"pointer",display:"flex",transition:"all .12s"}} onMouseEnter={e=>e.currentTarget.style.background=T.bg} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><Ic d={P.edit} sz={12} color={T.inkSub}/></button>}
                    {can(user,"delete")&&<button onClick={()=>onDelete(f.id)} style={{background:"transparent",border:`1px solid ${T.line}`,borderRadius:T.r.sm,padding:"4px 6px",cursor:"pointer",display:"flex",transition:"all .12s"}} onMouseEnter={e=>{e.currentTarget.style.background="#FEF2F2";e.currentTarget.style.borderColor="#FECACA";}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.borderColor=T.line;}}><Ic d={P.trash} sz={12} color="#DC2626"/></button>}
                  </div>
                </td>
              </tr>
            );
...
function Analytics({funnels}){
  ...
  const Row=({label,val,pct,color})=>(
    <div style={{marginBottom:14}}>
      ...
      <div style={{height:4,background:T.bg,borderRadius:2,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:2}}/></div>
    </div>
  );
  ...
  return(
    <div style={{padding:"20px 24px",display:"grid",gap:16}}>
      <div className="ek-analytics-3col" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
        ...
        <Card title="Revenue">{[["Won Potential",big(totalRevenue),T.won.dot]].map(([l,v,c])=><div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.line}`}}><span style={{fontSize:12,color:T.inkSub,fontFamily:F}}>{l}</span><span style={{fontSize:15,fontWeight:700,color:c,fontFamily:F,letterSpacing:"-0.3px"}}>{v}</span></div>)}</Card>
      </div>
      <div className="ek-analytics-3col" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
        <Card title="Leads by type"><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{FTYPES.map((t,i)=>{const n=funnels.filter(f=>f.funnelType===t).length;const colors=[T.brand,T.won.dot,T.pending.dot,T.premium.dot,T.new.dot];return<div key={t} style={{flex:1,minWidth:60,textAlign:"center",padding:"12px 8px",background:T.bg,borderRadius:T.r.md,border:`1px solid ${T.line}`}}><div style={{fontSize:22,fontWeight:700,color:colors[i]||T.brand,fontFamily:F}}>{n}</div><div style={{fontSize:10,color:T.inkMuted,marginTop:4,fontFamily:F,lineHeight:1.3}}>{t}</div></div>;})}></div></Card>
        <Card title="Leads by source">{bySrc.length===0?<div style={{fontSize:12,color:T.inkMuted,fontFamily:F}}>No source data yet.</div>:bySrc.map(({s,n})=>{const pct=funnels.length?Math.round(n/funnels.length*100):0;return<Row key={s} label={s} val={n} pct={pct} color={T.brand}/>;})}</Card>
        <Card title="Units by category">{byCat.map(({c,n})=><div key={c} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:T.ink,fontFamily:F}}>{c}</span><span style={{fontSize:12,fontWeight:600,color:T.ink,fontFamily:F}}>{n}</span></div><div style={{height:4,background:T.bg,borderRadius:2,overflow:"hidden"}}><div style={{width:`${Math.round(n/maxCat*100)}%`,height:"100%",background:T.brand,borderRadius:2,opacity:.7}}/></div></div>)}</Card>
      </div>
    </div>
  );
}
...
function Team({users,onSave}){
  ...
  const add=()=>{
    ...
    const u=[...list,{...form,id:Date.now()}];setList(u);onSave(u);setForm({name:"",username:"",password:"",role:"CRE"});setErr("");
  };
  ...
  return(
    <div className="ek-team-grid" style={{padding:"20px 24px",display:"grid",gridTemplateColumns:"360px 1fr",gap:20}}>
      <div style={{background:T.surface,border:`1px solid ${T.line}`,borderRadius:T.r.lg,padding:22,boxShadow:T.shadowSm}}>
        ...
        <div style={{marginTop:18,padding:"12px 14px",background:T.brandSubtle,borderRadius:T.r.md,fontSize:12,color:T.brand,lineHeight:1.7,fontFamily:F}}><strong>CEO & Manager</strong> — full access<br/><strong>CRE</strong> — create + export only</div>
      </div>
      <div style={{background:T.surface,border:`1px solid ${T.line}`,borderRadius:T.r.lg,padding:22,boxShadow:T.shadowSm}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {list.map(u=>{const c=rc[u.role]||T.drop;return(
            <div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",border:`1px solid ${T.line}`,borderRadius:T.r.md,transition:"background .1s"}} onMouseEnter={e=>e.currentTarget.style.background=T.bg} onMouseLeave={e=>e.currentTarget.style.background=T.surface}>
              <Avatar name={u.name} size={36}/><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:T.ink,fontFamily:F}}>{u.name}</div><div style={{fontSize:11,color:T.inkMuted,fontFamily:F}}>@{u.username}</div></div>
...
function FunnelForm({onClose,onSave,existing,user}){
  const blank={name:"",phone:"",email:"",enquiryType:"",funnelType:"",leadSource:"",cityRegion:"",nextFollowUp:"",products:[{desc:"",category:"",qty:"",price:""}],remarks:"",deliveryDetails:"",paymentTerms:"",orderNumber:"",quoteQty:"",quoteAmount:"",quoteDesc:"",status:"Pending"};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(2px)"}} onClick={onClose}>
      <div style={{background:T.surface,borderRadius:T.r["2xl"],width:"100%",maxWidth:720,maxHeight:"90vh",overflowY:"auto",boxShadow:T.shadowXl,animation:"fadeUp .2s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 24px 16px",borderBottom:`1px solid ${T.line}`,position:"sticky",top:0,background:T.surface,zIndex:1,borderRadius:`${T.r["2xl"]} ${T.r["2xl"]} 0 0`}}>
          ...
          <button onClick={onClose} style={{width:30,height:30,border:`1px solid ${T.line}`,borderRadius:T.r.md,background:T.bg,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={P.close} sz={13} color={T.inkSub}/></button>
        </div>
        <div style={{padding:"22px 24px",display:"flex",flexDirection:"column",gap:20}}>
          <section><SL>Contact details</SL>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <section><SL>Customer requirements</SL>
            <div style={{border:`1px solid ${errs.products?T.lost.dot:T.line}`,borderRadius:T.r.lg,overflow:"hidden"}}>
              <div style={{display:"grid",gridTemplateColumns:"2.5fr 1.4fr .8fr 1fr .35fr",padding:"8px 14px",background:T.bg,gap:8}}>
                {["Product / item *","Category","Qty","Unit price (Rs)",""].map(h=><div key={h} style={{fontSize:10,fontWeight:600,color:T.inkMuted,letterSpacing:"0.06em",fontFamily:F}}>{h}</div>)}
              </div>
              {form.products.map((pr,i)=>(
                <div key={i} style={{display:"grid",gridTemplateColumns:"2.5fr 1.4fr .8fr 1fr .35fr",padding:"9px 14px",borderTop:`1px solid ${T.line}`,gap:8,alignItems:"center"}}>
                  <input value={pr.desc} onChange={e=>sp(i,"desc",e.target.value)} placeholder="e.g. Bridal Lehenga" style={{...inputSx(),padding:"6px 9px",fontSize:12}} onFocus={onfocus} onBlur={onblur}/>
                  <select value={pr.category} onChange={e=>sp(i,"category",e.target.value)} style={{...inputSx(),padding:"6px 24px 6px 9px",fontSize:12,cursor:"pointer",appearance:"none",background:`${T.surface} ${selectBg}`}} onFocus={onfocus} onBlur={onblur}><option value="">Category</option>{CATS.map(c=><option key={c}>{c}</option>)}</select>
                  {[["qty","0"],["price","0"]].map(([k,ph])=><input key={k} type="number" value={pr[k]} onChange={e=>sp(i,k,e.target.value)} placeholder={ph} style={{...inputSx(),padding:"6px 9px",fontSize:12}} onFocus={onfocus} onBlur={onblur}/>)}
                  <button onClick={()=>set("products",form.products.filter((_,x)=>x!==i))} disabled={form.products.length===1} style={{background:"none",border:"none",cursor:form.products.length===1?"not-allowed":"pointer",color:T.inkMuted,fontSize:16,opacity:form.products.length===1?.2:1,display:"flex",alignItems:"center",justifyContent:"center"}}>x</button>
                </div>
              ))}
              <div style={{padding:"9px 14px",borderTop:`1px solid ${T.line}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <button onClick={()=>set("products",[...form.products,{desc:"",category:"",qty:"",price:""}])} style={{background:"none",border:`1px dashed ${T.brand}`,borderRadius:T.r.sm,padding:"4px 12px",color:T.brand,fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:F,display:"inline-flex",alignItems:"center",gap:5}}><Ic d={P.plus} sz={11} color={T.brand}/> Add item</button>
          ...
          <section><SL>Remarks <span style={{color:"#DC2626"}}>*</span></SL>
            <textarea value={form.remarks} onChange={e=>set("remarks",e.target.value)} placeholder="Additional notes…" rows={2} style={{...inputSx(errs.remarks),padding:"9px 11px",resize:"vertical",lineHeight:1.5}} onFocus={onfocus} onBlur={onblur}/>
            {errs.remarks&&<div style={{fontSize:11,color:"#B91C1C",marginTop:4}}>{errs.remarks}</div>}
          </section>
          <section><SL>Delivery & Payment</SL>
            <div className="ek-form-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                ...
                <textarea value={form.deliveryDetails} onChange={e=>set("deliveryDetails",e.target.value)} placeholder="e.g. Delivery by Apr 20, doorstep…" rows={2} style={{...inputSx(errs.deliveryDetails),padding:"9px 11px",resize:"vertical",lineHeight:1.5}} onFocus={onfocus} onBlur={onblur}/>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                ...
                <textarea value={form.paymentTerms} onChange={e=>set("paymentTerms",e.target.value)} placeholder="e.g. 50% advance, balance on delivery…" rows={2} style={{...inputSx(),padding:"9px 11px",resize:"vertical",lineHeight:1.5}} onFocus={onfocus} onBlur={onblur}/>
              </div>
            </div>
          </section>
          <section><SL>Initial quotation</SL>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div className="ek-form-3col" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                ...
                <FInput label="Amount (Rs)" type="number" value={form.quoteAmount} onChange={e=>set("quoteAmount",e.target.value)} placeholder="0" required error={errs.quoteAmount}/>
              </div>
              <div>
                ...
                <textarea value={form.quoteDesc} onChange={e=>set("quoteDesc",e.target.value)} placeholder="Quote notes…" rows={2} style={{...inputSx(errs.quoteDesc),padding:"9px 11px",resize:"vertical",lineHeight:1.5,width:"100%",boxSizing:"border-box"}} onFocus={onfocus} onBlur={onblur}/>
...
// Standalone component — always renders, no conditional gates, no prop crashes
function FollowUpLogSection({funnelId,funnelNextFollowUp,user,logs,onAddLog,onUpdateFunnelFollowUp}){
  const safeDate = funnelNextFollowUp && funnelNextFollowUp.match(/^\d{4}-\d{2}-\d{2}$/) ? funnelNextFollowUp : today();
  const [logForm,setLogForm]=useState({followUpDate:safeDate, description:"", nextFollowUp:""});
  const [logErr,setLogErr]=useState("");
  const [saving,setSaving]=useState(false);
  const roleColor={"CEO":T.high,"Manager":T.won,"CRE":T.pending};

  const submitLog=async()=>{
    if(!logForm.description.trim()){setLogErr("Please write what happened on this follow-up.");return;}
    setLogErr(""); setSaving(true);
    try{
      await onAddLog(funnelId,{
      ...
      if(logForm.nextFollowUp) onUpdateFunnelFollowUp(funnelId,logForm.nextFollowUp);
  ...
  return(
    <div style={{borderTop:`2px solid ${T.line}`,paddingTop:20,marginTop:24}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
        <div style={{width:28,height:28,borderRadius:T.r.md,background:"#F0FDF4",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <Ic d={P.cal} sz={14} color={T.won.dot}/>
        </div>
        <span style={{fontSize:13,fontWeight:700,color:T.ink,fontFamily:F}}>Follow-up Activity</span>
        {logs.length>0&&<span style={{fontSize:11,fontWeight:600,background:T.won.bg,color:T.won.text,padding:"2px 9px",borderRadius:20,fontFamily:F}}>{logs.length}</span>}
        <span style={{marginLeft:"auto",fontSize:11,color:T.inkMuted,fontFamily:F}}>All roles can log</span>
      </div>

      {/* Input card */}
      <div style={{background:"#FAFFFE",border:"1px solid rgba(22,163,74,0.18)",borderRadius:T.r.lg,padding:16,marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:600,color:T.won.text,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:12,fontFamily:F}}>Log outcome</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            <label style={{fontSize:11,fontWeight:500,color:T.inkSub,fontFamily:F}}>Follow-up date covered</label>
            <input type="date" value={logForm.followUpDate} onChange={e=>setLogForm(f=>({...f,followUpDate:e.target.value}))} style={{...inputSx(),fontSize:12,padding:"7px 9px"}} onFocus={onfocus} onBlur={onblur}/>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            <label style={{fontSize:11,fontWeight:500,color:T.inkSub,fontFamily:F}}>Next follow-up <span style={{color:T.inkMuted,fontWeight:400}}>(optional)</span></label>
            <input type="date" value={logForm.nextFollowUp} onChange={e=>setLogForm(f=>({...f,nextFollowUp:e.target.value}))} style={{...inputSx(),fontSize:12,padding:"7px 9px"}} onFocus={onfocus} onBlur={onblur}/>
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          <label style={{fontSize:11,fontWeight:500,color:T.inkSub,fontFamily:F}}>What happened / outcome <span style={{color:"#DC2626"}}>*</span></label>
          <textarea value={logForm.description} onChange={e=>{setLogForm(f=>({...f,description:e.target.value}));setLogErr("");}} placeholder="e.g. Called customer — interested but asking for 10% discount…" rows={3} style={{...inputSx(!!logErr),padding:"9px 11px",resize:"vertical",lineHeight:1.6,fontSize:12,fontFamily:F}} onFocus={onfocus} onBlur={onblur}/>
          {logErr&&<span style={{fontSize:11,color:"#B91C1C",fontFamily:F}}>{logErr}</span>}
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:12}}>
          <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:T.inkMuted,fontFamily:F}}>
            <Avatar name={user.name} size={18}/>
            <span>Logging as <strong style={{color:T.ink}}>{user.name}</strong></span>
          </div>
          <Btn primary sm icon={P.check} label={saving?"Saving…":"+ Log Activity"} onClick={submitLog} disabled={!logForm.description.trim()||saving}/>
        </div>
      </div>

      {/* Log list */}
      {logs.length===0
        ? <div style={{textAlign:"center",padding:"20px 0",fontSize:12,color:T.inkMuted,fontFamily:F,background:T.bg,borderRadius:T.r.lg,border:`1px dashed ${T.line}`}}>No activity logged yet. Log your first outcome above.</div>
        : <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {logs.map((lg,i)=>{
              const rc=roleColor[lg.role]||T.drop;
              return(
                <div key={i} style={{background:T.surface,border:`1px solid ${T.line}`,borderRadius:T.r.lg,padding:"13px 15px",boxShadow:T.shadowSm}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    <Avatar name={lg.author} size={26}/>
                    <div style={{flex:1}}>
                  ...
                  <div style={{display:"inline-flex",alignItems:"center",gap:4,background:T.bg,border:`1px solid ${T.line}`,borderRadius:20,padding:"2px 9px",marginBottom:8}}>
                    <Ic d={P.cal} sz={10} color={T.inkMuted}/>
                    <span style={{fontSize:10,color:T.inkSub,fontFamily:F}}>Follow-up for:</span>
                    <span style={{fontSize:11,fontWeight:600,color:T.ink,fontFamily:F,marginLeft:3}}>{lg.followUpDate}</span>
                  </div>
                  <p style={{margin:0,fontSize:13,color:T.ink,fontFamily:F,lineHeight:1.65}}>{lg.description}</p>
                  {lg.nextFollowUp&&(
                    <div style={{marginTop:10,display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:T.brandSubtle,borderRadius:T.r.md}}>
                      <Ic d={P.arrow} sz={12} color={T.brand}/>
                      <span style={{fontSize:12,color:T.brand,fontFamily:F,fontWeight:500}}>Next follow-up set to <strong>{lg.nextFollowUp}</strong></span>
                    </div>
                  )}
                </div>
              );
...
function ViewDrawer({funnel,onClose,onEdit,onStatusChange,user,comments,onAddComment,logs,onAddLog,onUpdateFunnelFollowUp}){
  ...
  // Always guarantee arrays — this is the main fix
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",zIndex:2000,display:"flex",justifyContent:"flex-end",backdropFilter:"blur(1px)"}} onClick={onClose}>
      <div style={{background:T.surface,width:"100%",maxWidth:540,height:"100%",overflowY:"auto",boxShadow:"-8px 0 40px rgba(0,0,0,.12)",animation:"slideRight .22s ease",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"20px 22px 16px",borderBottom:`1px solid ${T.line}`,position:"sticky",top:0,background:T.surface,zIndex:1}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:12}}>
            <div style={{flex:1,marginRight:12}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}><p style={{margin:0,fontSize:11,color:T.inkMuted,fontFamily:F}}>{funnel.createdAt} · {funnel.createdBy}</p>{funnel.leadSource&&<SourcePill source={funnel.leadSource}/>}</div>
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {can(user,"edit")&&<Btn ghost sm icon={P.edit} label="Edit" onClick={()=>{onClose();onEdit(funnel);}}/>}
              <button onClick={onClose} style={{width:28,height:28,border:`1px solid ${T.line}`,borderRadius:T.r.md,background:T.bg,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={P.close} sz={12} color={T.inkSub}/></button>
            </div>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {STATUS.map(s=>{const c=T[s.toLowerCase()]||T.drop;const a=status===s;return(
              <button key={s} onClick={()=>doStatus(s)} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${a?c.dot:T.line}`,background:a?c.bg:"transparent",color:a?c.text:T.inkSub,fontSize:12,fontWeight:a?600:400,cursor:"pointer",fontFamily:F,transition:"all .15s",display:"flex",alignItems:"center",gap:5}}>
        ...
        <div style={{padding:"18px 22px",flex:1}}>
          <Sec t="Contact"/>
          <dl><Row l="Name" v={funnel.name}/><Row l="Phone" v={funnel.phone}/><Row l="Email" v={funnel.email}/>{funnel.cityRegion&&<Row l="City / Region" v={funnel.cityRegion}/>}</dl>
          <div style={{height:18}}/>

          <Sec t="Funnel"/>
          <dl><Row l="Enquiry type" v={funnel.enquiryType}/><Row l="Funnel type" v={funnel.funnelType}/><Row l="Lead source" v={funnel.leadSource}/><Row l="Next follow-up" v={funnel.nextFollowUp}/></dl>
          <div style={{height:18}}/>
          ...
          {/* ── FOLLOW-UP ACTIVITY LOG — always rendered, no conditions ── */}
          <FollowUpLogSection
            funnelId={funnel.id}
            funnelNextFollowUp={funnel.nextFollowUp}
            user={user}
            logs={safeLogs}
            onAddLog={onAddLog}
            onUpdateFunnelFollowUp={onUpdateFunnelFollowUp}
          />

          {/* ── AUDIT COMMENTS ── */}
          <div style={{borderTop:`2px solid ${T.line}`,paddingTop:20,marginTop:24}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
              <div style={{width:28,height:28,borderRadius:T.r.md,background:T.brandSubtle,display:"flex",alignItems:"center",justifyContent:"center"}}><Ic d={P.msg} sz={14} color={T.brand}/></div>
              <span style={{fontSize:13,fontWeight:700,color:T.ink,fontFamily:F}}>Audit Comments</span>
              {safeComments.length>0&&<span style={{fontSize:11,fontWeight:600,background:T.brandSubtle,color:T.brand,padding:"2px 9px",borderRadius:20,fontFamily:F}}>{safeComments.length}</span>}
              {!canComment&&<span style={{marginLeft:"auto",fontSize:11,color:T.inkMuted,fontFamily:F}}>CEO / Manager only</span>}
            </div>
            {canComment&&(
              <div style={{marginBottom:16}}>
                <textarea value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Write your audit comment…" rows={3} style={{...inputSx(),padding:"10px 12px",resize:"vertical",lineHeight:1.6,width:"100%",boxSizing:"border-box"}} onFocus={onfocus} onBlur={onblur}/>
                <div style={{display:"flex",justifyContent:"flex-end",marginTop:8}}><Btn primary sm icon={P.check} label="Save comment" onClick={submitComment} disabled={!commentText.trim()}/></div>
              </div>
            )}
            {safeComments.length===0
              ? <div style={{textAlign:"center",padding:"20px 0",fontSize:12,color:T.inkMuted,fontFamily:F}}>No audit comments yet.</div>
              : <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {[...safeComments].reverse().map((c,i)=>{
                    ...
                    return(
                      <div key={i} style={{background:T.bg,border:`1px solid ${T.line}`,borderRadius:T.r.lg,padding:"12px 14px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                          ...
                          <div style={{flex:1}}><span style={{fontSize:12,fontWeight:600,color:T.ink,fontFamily:F}}>{c.author}</span><span style={{marginLeft:6,fontSize:10,fontWeight:500,padding:"1px 7px",borderRadius:10,background:rc.bg,color:rc.text,fontFamily:F}}>{c.role}</span></div>
...
function Shell({user,users,onLogout,onUsersChange}){
  ...
  // Funnels fetch
  useEffect(()=>{
    crmService.getAllFunnels()
      .then(data=>setFunnels(data))
      .catch(err=>console.error("Fetch funnels:",err))
      .finally(()=>setLoading(false));
  },[]);

  // Funnels realtime
  useEffect(()=>{
    const ch=supabase.channel('funnels_changes')
      .on('postgres_changes',{event:'*',schema:'public',table:'funnels'},(payload)=>{
        if(payload.eventType==='INSERT'){const f=crmService.mapFromDb(payload.new);setFunnels(prev=>prev.find(x=>x.id===f.id)?prev:[f,...prev]);}
        else if(payload.eventType==='UPDATE'){const f=crmService.mapFromDb(payload.new);setFunnels(prev=>prev.map(x=>x.id===f.id?f:x));}
        else if(payload.eventType==='DELETE'){setFunnels(prev=>prev.filter(x=>x.id!==payload.old.id));}
      }).subscribe();
  ...
  useEffect(()=>{
    if(!viewT) return;
    crmService.getComments(viewT.id)
      .then(c=>setFunnelComments(prev=>({...prev,[viewT.id]:c})))
      .catch(()=>setFunnelComments(prev=>({...prev,[viewT.id]:[]})));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[viewT?.id]);

  useEffect(()=>{
    const ch=supabase.channel('comments_changes')
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'audit_comments'},(payload)=>{
        const fid=payload.new.funnel_id;
        const nc={text:payload.new.text,author:payload.new.author,role:payload.new.role,time:new Date(payload.new.created_at).toLocaleString('en-IN',{month:'short',day:'numeric',year:'numeric',hour:'2-digit',minute:'2-digit'})};
  ...
  // Re-fetch every time a different funnel is opened (viewT?.id as dep)
  useEffect(()=>{
    if(!viewT) return;
    const id=viewT.id;
    crmService.getFollowUpLogs(id)
      .then(logs=>{
        setFunnelLogs(prev=>({...prev,[id]:Array.isArray(logs)?logs:[]}));
      })
      .catch(err=>{
        console.error("getFollowUpLogs failed:",err);
        setFunnelLogs(prev=>({...prev,[id]:[]})); // always set to [] so section renders
      });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[viewT?.id]);

  // Realtime for follow_up_logs inserts
  useEffect(()=>{
    const ch=supabase.channel('followup_logs_rt')
  ...
  const addFollowUpLog=async(funnelId,log)=>{
    await crmService.addFollowUpLog(funnelId,log); // errors bubble to FollowUpLogSection
  };
  ...
  const updateFunnelFollowUp=async(funnelId,newDate)=>{
    ...
    push(`Follow-up updated to ${newDate}`);
    // Try DB update (non-fatal if fails)
    try{
      if(crmService.updateFollowUpDate) await crmService.updateFollowUpDate(funnelId,newDate);
    }catch(err){console.error("updateFollowUpDate (non-fatal):",err);}
  };
    ...
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:F}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,minHeight:"100vh"}}>
        <Topbar title={titles[view]} search={search} setSearch={setSearch} user={user} onAdd={()=>setAddOpen(true)}
          ...
          fLen={filtered.length} aLen={scoped.length} onMenuToggle={()=>setSidebarOpen(x=>!x)}/>
        ...
        <div style={{flex:1,background:showFilters?T.surface:"transparent",borderTop:showFilters?`1px solid ${T.line}`:"none"}}>
          {(view==="dashboard"||view==="funnels")&&<Table rows={filtered} user={user} onView={setViewT} onEdit={f=>setEditT(f)} onDelete={del} loading={loading}/>}
      ...
      {viewT&&(
        <ViewDrawer
          ...
          user={user}
          comments={Array.isArray(funnelComments[viewT.id])?funnelComments[viewT.id]:[]}
          onAddComment={addComment}
          logs={Array.isArray(funnelLogs[viewT.id])?funnelLogs[viewT.id]:[]}
...
export default function App(){
  ...
  useEffect(()=>{
    crmService.getUsers()
      .then(data=>setUsers(data&&data.length>0?data:SEED_USERS))
  ...
  const handleUsersChange=async(newUsers)=>{
    try{
      ...
      setUsers(data);
    }catch(err){console.error("handleUsersChange:",err);}
  };
  ...
  return(
    <><FontLoader/>{!user?<Login users={users} onLogin={setUser}/>:<Shell user={user} users={users} onLogout={()=>setUser(null)} onUsersChange={handleUsersChange}/>}</>
  );
}
